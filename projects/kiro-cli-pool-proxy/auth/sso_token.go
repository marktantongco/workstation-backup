...
// ImportFromSsoToken imports an account from an SSO Token (x-amz-sso_authn bearer).
func ImportFromSsoToken(bearerToken, region string) (accessToken, refreshToken, clientID, clientSecret string, expiresIn int, err error) {
	...
	// 4. Get the device session token.
...
// GetUserInfo fetches user info (email, userID) via the Kiro usage-limits API.
func GetUserInfo(accessToken string) (email, userID string, err error) {
	// Call the Kiro API to fetch usage info (which includes user info).
