...
// Refresh fetches a fresh proxy list from the configured URL.
func (p *ProxyPool) Refresh(ctx context.Context) error {
	...
	log.Printf("[proxy-pool] Refreshed: %d healthy proxies (strict_geo=%v, geo_verify=%v)", len(healthy), p.strictGeo, p.geoVerify)
	return nil
}
...
func (p *ProxyPool) Stats() any {
	...
	for _, e := range p.proxies {
		...
		if e.Country != "" {
			s["by_country"].(map[string]int)[e.Country]++
		}
	}
	s["alive"] = alive
	s["dead"] = len(p.proxies) - alive
	return s
}
...
func (p *ProxyPool) geoVerifyProxies(ctx context.Context, entries []*ProxyEntry) []*ProxyEntry {
	...
	httpReq, err := http.NewRequestWithContext(verifyCtx, "POST", "http://ip-api.com/batch", bytes.NewReader(body))
	if err != nil {
