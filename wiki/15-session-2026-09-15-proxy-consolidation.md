...
- **ADMIN_TOKEN rotated** (`123456` → non-default, set via the bootstrap-exempt `POST /admin/api/change-password`): dual-layer write verified (`.env` seed + settings overlay `source: db`, mode 0600, owner x1), `DASHBOARD_REQUIRE_LOGIN=true` converged, old token 401s, old session cookies invalidated, new credential logs in. Public default-password WARN will no longer fire. Note for the vault: credential is `admin1234` — rotate it into the token store.
